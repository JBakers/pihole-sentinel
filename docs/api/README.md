...
The API key is automatically generated by `install.py` during deployment and injected
...
async function getHistory(hours = 24) {
    const response = await fetch(`${API_BASE}/api/history?hours=${hours}`, { headers });
